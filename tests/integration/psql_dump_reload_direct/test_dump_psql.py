    ...
    with Flow.load_config(storage_flow_yml) as flow_storage:
        with Flow.load_config(query_flow_yml) as flow_query:
            ...
            with TimeContext(f'### dumping {len(docs)} docs'):
                # TODO replace with automatic snapshotting inside PSQL?
                flow_storage.post(
                    on='/snapshot',
                    target_peapod='indexer_storage',
                )

            # TODO rolling update is called on the executor directly?
            # TODO dump_path is string to PSQL connection
            # TODO Indexer gets data from PSQL --- todo in commons
            flow_query.rolling_update(
                pod_name='indexer_query',
                # must pass urllib.parse.urlparse
                dump_path='postgres://default_name:default_pwd@localhost:5432/postgres?table=snapshot'
            )
